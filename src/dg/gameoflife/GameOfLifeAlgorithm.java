...
* out the algorithm to simulate the rules of the game.
*
...
public class GameOfLifeAlgorithm {
	...
	// Creates an updated grid of the current grid after applying the rules of 
	...
	public static void createUpdatedGrid(Grid currentGrid) {
		...
		// Loops through each cell fo the grid and checks if it is dead or alive.
	...
	// Creates a Grid object which contains a 30 by 30 grid of Rectangles; all white.
	...
	public static void liveCellNeighbourChecker(Grid currentGrid, Grid tempGrid, int x, int y) {
		...
		for (int col = (x-1); col<(x+2); col++) {
			for (int row = (y-1); row<(y+2); row++) {
				if (currentGrid.cellStatus(col, row) == true)
					numberOfNeighbours++;
			}
		}
